...
public:
    ...
    : nextSample (0), subSample (0), accumulator (0), sampleRate (44100), bufferSize (512)
    {
        setOpaque (true);
        clear();
        ...
        for(int i = 0; i < bufferSize; i++) { buffer_1[i]= 0;}
                                ...
                                int numSamples) override
    {
        for (int i = 0; i < numSamples; ++i)
        {
            ...
            sample_for_mapping = 10.0f * inputSample; // for mapping
            
            for (int i = 0; i < bufferSize; i++){
                buffer_1[i]	= inputSample;
            }
        }
    ...
    float get_rms(){
        return (audioAnalyzer1.getRms());
    }

    float get_energy(){
        return (audioAnalyzer1.getEnergy());
    }


    // -----------------------------------------------------------------

    SimpleAudioMIReactiveUtil audioAnalyzer1;
